...
points(bankdata[index, 1], bankdata[index, 2], col = "red", cex = 4, pch = 4)
...
This gives 16203 as minimum function value but is painfully slow so cannot run it in the knit.
...
```{r 2c2}
set.seed(1234)
SA(prob_keep = 0.9, alpha = 0.7, beta = 1.03, mj = 50, temp = 10000, it = 50)


```

Higher chance of swapping individuals seems to lead to a worse outcome
```{r 2c3}
set.seed(1234)
SA(prob_keep = 0.7, alpha = 0.7, beta = 1.03, mj = 50, temp = 10000, it = 50)

```

Lower swaprate, on average 1.1 individual performed worse than swapping 2 on average, swapping fewer individuals might lead to locked local minima situations or slower progress. 
```{r 2c4}
set.seed(12345)
SA(prob_keep = 0.95, alpha = 0.7, beta = 1.05, mj = 25, temp = 10000, it = 50)
