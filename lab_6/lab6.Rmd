...
```
It looks like they converged
```{r 1d}
em <- emalg(dat3p, eps=0.000001)
...
points(bankdata[index, 1], bankdata[index, 2], col = "red")
...
Lower function value and faster, higher starting temperature and lower alpha makes it decay faster, 0.9 probability swaps 2 people on average.

```{r 2c2}
set.seed(1234)
SA(prob_keep = 0.9, alpha = 0.7, beta = 1.03, mj = 50, temp = 100000, it = 50)


```

Higher chance of swapping seems to lead to a worse outcome
```{r 2c3}
set.seed(1234)
SA(prob_keep = 0.7, alpha = 0.7, beta = 1.03, mj = 50, temp = 100000, it = 50)

```

Lower swaprate seems better.
```{r 2c4}
set.seed(12345)
SA(prob_keep = 0.95, alpha = 0.7, beta = 1.05, mj = 25, temp = 100000, it = 50)
